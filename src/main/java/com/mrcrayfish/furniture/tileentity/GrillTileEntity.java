...
public class GrillTileEntity extends TileEntity implements IClearable, ITickableTileEntity, ISidedInventory {
        ...
        if (!this.world.isRemote()) {
            ...
            if (this.remainingFuel == 0 && canCook) {
                for (int i = this.fuel.size() - 1; i >= 0; i--) {
                    if (!this.fuel.get(i).isEmpty()) {
                        this.remainingFuel = 1000; // TODO net.minecraftforge.common.ForgeHooks.getBurnTime(this.fuel.get(i));
    ...
    public void read(BlockState blockState, CompoundNBT compound) {
        ...
        if (compound.contains("Grill", NbtTagNumbers.TAG_LIST)) {
            this.grill.clear();
            ItemStackHelper.loadAllItems("Grill", compound, this.grill);
        }
        if (compound.contains("Fuel", NbtTagNumbers.TAG_LIST)) {
            this.fuel.clear();
            ItemStackHelper.loadAllItems("Fuel", compound, this.fuel);
        }
        if (compound.contains("RemainingFuel", NbtTagNumbers.TAG_INT)) {
            this.remainingFuel = compound.getInt("RemainingFuel");
        }
        if (compound.contains("CookingTimes", NbtTagNumbers.TAG_INT_ARRAY)) {
        ...
        if (compound.contains("CookingTotalTimes", NbtTagNumbers.TAG_INT_ARRAY)) {
        ...
        if (compound.contains("Flipped", NbtTagNumbers.TAG_BYTE_ARRAY)) {
        ...
        if (compound.contains("Experience", NbtTagNumbers.TAG_INT_ARRAY)) {
        ...
        if (compound.contains("Rotations", NbtTagNumbers.TAG_BYTE_ARRAY)) {
